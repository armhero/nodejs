...
node('armv7') {
  ...
  stage('Build') {
    sh 'sudo docker build -t armhero/nodejs:\044{BRANCH_NAME} .'
  }
    ...
    withCredentials([
      ...
      sh '''#!/bin/bash -xe
        ...
        if [[ "\044{BRANCH_NAME}" == "master" ]]; then
          ...
          sudo docker tag armhero/nodejs:\044{BRANCH_NAME} armhero/nodejs:latest

          sudo docker push armhero/nodejs:latest
        else
          sudo docker push armhero/nodejs:${BRANCH_NAME}
        fi
      '''
    }
  }
